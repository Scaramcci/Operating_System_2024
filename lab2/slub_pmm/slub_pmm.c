...
static void
slub_pmm_init_memmap(struct Page* base, size_t n) {
    assert(n > 0);
    struct Page* p = base;
    for (; p != base + n; p++) {
...
static void*
slub_alloc_blocks(struct Page* page, size_t size) {
...
static void
slub_pmm_free_pages(struct Page* base, size_t n) {
    assert(n > 0);
    struct Page* p = base;
    for (; p != base + n; p++) {
